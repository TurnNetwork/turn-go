...
import (
	...
	"github.com/bubblenet/bubble/light"
...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	for i, hash := range res.hashes {
		...
		slim := snapshot.SlimAccountRLP(res.accounts[i].Nonce, res.accounts[i].Balance, res.accounts[i].Root, res.accounts[i].CodeHash, res.accounts[i].StorageKeyPrefix)
