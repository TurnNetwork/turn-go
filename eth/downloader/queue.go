...
func newFetchResult(header *types.Header, fastSync bool) *fetchResult {
	...
	item.pending |= (1 << bodyType) // bubble needs to deliver ExtraData
	//if !header.EmptyBody() {
	//	item.pending |= (1 << bodyType)
	//}
	if fastSync && !header.EmptyReceipts() {
		//item.pending |= (1 << receiptType)	// The receipt is not synchronized in bubble SnapSync mode, so comment here
	}
	return item
}
...
func newQueue(blockCacheLimit int, thresholdInitialSize int, decodeExtra decodeExtraFn) *queue {
