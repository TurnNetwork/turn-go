...
func TestTraceCall(t *testing.T) {
	...
	genesis := &core.Genesis{Alloc: core.GenesisAlloc{
		accounts[0].addr: {Balance: big.NewInt(params.BUB)},
		accounts[1].addr: {Balance: big.NewInt(params.BUB)},
		accounts[2].addr: {Balance: big.NewInt(params.BUB)},
	}}
	genBlocks := 10
		...
		// Succcessful call with state overriding
		{
			...
			config: &TraceCallConfig{
				...
				StateOverrides: &ethapi.StateOverride{
					randomAccounts[0].addr: ethapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.BUB)))},
				},
			},
			expectErr: nil,
			expect: &callTrace{
