...
import (
	...
	"github.com/bubblenet/bubble/p2p/dnsdisc"
	"math/big"
	"os"
	"strconv"
	"strings"
	...
	vrfhandler "github.com/bubblenet/bubble/x/handler"
...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	if config.SyncMode == downloader.SnapSync {
		return nil, errors.New("can't run bubble in snap sync mode now")
	}
	...
	log.Info("Initialising bubble protocol", "network", config.NetworkId, "dbversion", dbVer)
	...
	if engine, ok := eth.engine.(consensus.Bft); ok {
		...
		if chainConfig.Cbft.ValidatorMode == "" || chainConfig.Cbft.ValidatorMode == common.STATIC_VALIDATOR_MODE {
			...
			handlePlugin(reactor, chainDb, chainConfig, config.DBValidatorsHistory, config.OpPriKey)
			agency = reactor
	...
	eth.netRPCService = ethapi.NewPublicNetAPI(eth.p2pServer, xplugin.BubbleInstance(), config.NetworkId)
...
func handlePlugin(reactor *core.BlockChainReactor, chainDB ethdb.Database, chainConfig *params.ChainConfig, isValidatorsHistory bool, opPrikey string) {
	...
	reactor.RegisterPlugin(xcom.StakingL2Rule, xplugin.StakingL2Instance())
