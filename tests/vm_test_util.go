...
func (t *VMTest) Run(vmconfig vm.Config) error {
	...
	for addr, account := range t.json.Post {
		for k, wantV := range account.Storage {
			if haveV := statedb.GetState(addr, k.Bytes()); bytes.Equal(haveV, wantV.Bytes()) {
...
func (t *VMTest) exec(statedb *state.StateDB, vmconfig vm.Config) ([]byte, uint64, error) {
	...
	e := t.json.Exec
	return evm.Call(vm.InvokedByTx, vm.AccountRef(e.Caller), e.Address, e.Data, e.GasLimit, e.Value)
}
...
func (t *VMTest) newEVM(statedb *state.StateDB, vmconfig vm.Config) *vm.EVM {
	...
	return vm.NewEVM(context, txContext, nil, statedb, params.MainnetChainConfig, vmconfig)
}
