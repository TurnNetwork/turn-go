...
func (wal *baseWal) LoadChainState(recovery recoveryChainStateFn) error {
	...
	if err != nil {
		log.Warn("Failed to get chainState from db, may be the first time to run bubble")
		return nil
	}
...
func (wal *baseWal) Load(recovery recoveryConsensusMsgFn) error {
	...
	if err != nil {
		log.Warn("Failed to get viewChange meta from db, may be the first time to run bubble")
		return nil
	}
