...
func gasSStoreEIP2929(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	var (
		...
		current = evm.StateDB.GetState(contract.Address(), slot.Bytes())
		cost    = uint64(0)
	)
	...
	if bytes.Equal(current, value.Bytes()) { // noop (1)
		// EIP 2200 original clause:
		//		return params.SloadGasEIP2200, nil
		return cost + WarmStorageReadCostEIP2929, nil // SLOAD_GAS
	}
	original := evm.StateDB.GetCommittedState(contract.Address(), common.Hash(x.Bytes32()).Bytes())
	if bytes.Equal(current, original) {
		if bytes.Equal(common.Hash{}.Bytes(), original) { // create slot (2.1.1)
	...
	if !bytes.Equal(original, (common.Hash{}).Bytes()) {
		if bytes.Equal(current, (common.Hash{}).Bytes()) { // recreate slot (2.2.1.1)
			evm.StateDB.SubRefund(params.SstoreClearsScheduleRefundEIP2200)
		} else if value == (common.Hash{}) { // delete slot (2.2.1.2)
			evm.StateDB.AddRefund(params.SstoreClearsScheduleRefundEIP2200)
		}
	}
	if bytes.Equal(original, value.Bytes()) {
		if bytes.Equal(common.Hash{}.Bytes(), original) { // reset to original inexistent slot (2.2.2.1)
			// EIP 2200 Original clause:
