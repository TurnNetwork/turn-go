...
func TestTransactionEncode(t *testing.T) {
	...
	txb1, err := rightvrsTx.MarshalBinary()
	if err != nil {
		t.Fatalf("marshal binary error: %v", err)
	}
	if !bytes.Equal(txb1, should) {
		t.Errorf("marshal binary mismatch, got %x", txb)
	}
}
...
func TestEIP2718TransactionSigHash(t *testing.T) {
	...
	if s.Hash(signedEip2718Tx, nil) != common.HexToHash("49b486f0ec0a60dfbbca2d30cb07c9e8ffb2a2ff41f29a1ab6737475f6ff69f3") {
		t.Errorf("signed EIP-2718 transaction hash mismatch, got %x", s.Hash(signedEip2718Tx, nil))
	}
}
...
func TestEIP2930Signer(t *testing.T) {
	...
	for i, test := range tests {
		sigHash := test.signer.Hash(test.tx, nil)
...
func EncodeRecipientEmpty() (string, error) {
	...
	err = rlp.Encode(buffer, tx.inner)
	if err != nil {
...
func TestRecipientNormal(t *testing.T) {
	...
	buffer := new(bytes.Buffer)
	err := rlp.Encode(buffer, txtmp.inner)
	if err != nil {
		fmt.Println("geninput fail.", err)
	}
	str := common.Bytes2Hex(buffer.Bytes())
